...
def _resolve_wa_sender_credentials(state, *lookup_ids: Optional[str]) -> tuple[Optional[str], Optional[str]]:
    memory_manager = getattr(state, "memory_manager", None)
    if not memory_manager:
        return None, None
    for lookup_id in lookup_ids:
        key = str(lookup_id or "").strip()
        if not key:
            continue
        try:
            phone_id = memory_manager.get_flag(key, "whatsapp_phone_id")
            token = memory_manager.get_flag(key, "whatsapp_token")
...
def _check_meta_whatsapp_contact(
    *,
    ...
    """
    Devuelve:
    - (False, code, details) si Meta confirma que NO tiene WhatsApp.
    - (True, None, None) si Meta confirma válido.
    - (None, None, None) si no se pudo validar (no bloquear flujo).
    """
    if not phone_id or not token or not recipient_phone:
        return None, None, None

    url = f"https://graph.facebook.com/v19.0/{phone_id}/contacts"
    headers = {
    ...
    payload = {
        ...
        "blocking": "wait",
        "force_check": True,
        "contacts": [recipient_phone],
    }
    try:
        resp = requests.post(url, headers=headers, json=payload, timeout=10)
    except Exception as exc:
        log.warning("No se pudo validar contacto WA en Meta (se continúa flujo): %s", exc)
        return None, None, None
    ...
    if resp.status_code >= 400:
        ...
        error_details = (
            (error_obj.get("error_data") or {}).get("details")
            if isinstance(error_obj.get("error_data"), dict)
            else None
        )
        error_details = str(error_details or error_obj.get("message") or resp.text or "").strip() or None
        if error_code == "131026" or "not on whatsapp" in str(error_details or "").lower():
            return False, error_code or "131026", error_details or "Phone number not on WhatsApp"
        log.warning(
            "Meta contacts check respondió %s (se continúa flujo): code=%s details=%s",
            resp.status_code,
            error_code,
            error_details,
        )
        return None, error_code, error_details
    ...
    if isinstance(contacts, list) and contacts:
        ...
        if status in {"invalid", "not_valid"}:
            return False, "131026", "Phone number not on WhatsApp"
...
def register_template_routes(app, state) -> None:
    ...
    async def send_template(
        ...
        try:
            ...
            wa_phone_id, wa_token = _resolve_wa_sender_credentials(
                state,
                context_id,
                session_id,
                chat_id,
            )
            is_valid_contact, contact_error_code, contact_error_details = _check_meta_whatsapp_contact(
                phone_id=wa_phone_id,
                token=wa_token,
                recipient_phone=chat_id,
            )
            if is_valid_contact is False:
                log.warning(
                    "[TEMPLATE_SEND] rejected_no_whatsapp chat_id=%s recipient=%s code=%s details=%s",
                    chat_id,
                    payload.recipient.phone,
                    contact_error_code or "131026",
                    contact_error_details or "Phone number not on WhatsApp",
                )
                raise HTTPException(
                    status_code=422,
                    detail={
                        ...
                        "code": "wa_no_account",
                        "provider": "meta",
                        "provider_error_code": contact_error_code or "131026",
                        "message": "This number has no WhatsApp account",
                        "recipient": chat_id,
                        "details": contact_error_details or "Phone number not on WhatsApp",
                    },
                )
