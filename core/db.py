...
# ======================================================
# 📤 WhatsApp outbox (pendientes/confirmaciones)
# ======================================================
WA_OUTBOX_TABLE = "wa_outbox_messages"


def create_wa_outbox_message(
    *,
    provider: str = "meta",
    ...
    clean_provider_id = str(provider_message_id or "").strip()
    if not clean_provider_id:
        return None

    data: dict[str, Any] = {
        "provider": str(provider or "meta").strip() or "meta",
    ...
    if context_id:
        data["context_id"] = str(context_id).strip()
    if property_id is not None:
        data["property_id"] = str(property_id).strip()
    if instance_id:
        data["instance_id"] = str(instance_id).strip()
    if template_name:
        data["template_name"] = str(template_name).strip()
    if template_language:
        data["template_language"] = str(template_language).strip()
    if payload is not None:
        data["payload"] = payload
    if rendered_text:
        data["rendered_text"] = str(rendered_text)

    try:
        resp = (
            supabase.table(WA_OUTBOX_TABLE)
            .upsert(data, on_conflict="provider_message_id")
            .execute()
        )
        rows = resp.data or []
        if rows:
            return rows[0]
        return find_wa_outbox_message(clean_provider_id)
    except Exception as exc:
        logging.warning("⚠️ No se pudo crear outbox WA (%s): %s", clean_provider_id, exc)
        return None


def find_wa_outbox_message(provider_message_id: str) -> dict | None:
    clean_provider_id = str(provider_message_id or "").strip()
    if not clean_provider_id:
        return None
    try:
        resp = (
            supabase.table(WA_OUTBOX_TABLE)
            .select("*")
            .eq("provider_message_id", clean_provider_id)
            .limit(1)
            .execute()
        )
        rows = resp.data or []
        ...
        logging.warning("⚠️ No se pudo leer outbox WA (%s): %s", clean_provider_id, exc)
        return None


def update_wa_outbox_message(
    provider_message_id: str,
    *,
    status: str | None = None,
    error_code: str | int | None = None,
    error_details: str | None = None,
    last_webhook_payload: dict | None = None,
) -> dict | None:
    clean_provider_id = str(provider_message_id or "").strip()
    if not clean_provider_id:
        return None
    ...
    if error_details:
        updates["error_details"] = str(error_details).strip()
    if last_webhook_payload is not None:
        updates["last_webhook_payload"] = last_webhook_payload
    ...
    try:
        ...
        logging.warning("⚠️ No se pudo actualizar outbox WA (%s): %s", clean_provider_id, exc)
        return None


def mark_wa_outbox_message_visible(
    provider_message_id: str,
    *,
    visible_message_content: str | None = None,
) -> bool:
    clean_provider_id = str(provider_message_id or "").strip()
    if not clean_provider_id:
        return False
    ...
    updates: dict[str, Any] = {
        "visible_message_created_at": now_iso,
        "updated_at": now_iso,
    }
    if visible_message_content:
        updates["visible_message_content"] = str(visible_message_content)
    ...
    try:
        resp = (
            ...
            .update(updates)
            .eq("provider_message_id", clean_provider_id)
            .is_("visible_message_created_at", "null")
            .execute()
        )
        ...
        try:
            ...
            logging.warning("⚠️ No se pudo marcar outbox visible (%s): %s", clean_provider_id, exc)
            return False
