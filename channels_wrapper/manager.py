...
class ChannelManager:
    ...
    async def check_recipient_has_whatsapp_account(
        ...
        *,
        ...
        raw_precheck_flag = os.getenv("WA_CONTACTS_PRECHECK_ENABLED")
        if raw_precheck_flag is None:
            precheck_enabled = bool(getattr(C, "WA_CONTACTS_PRECHECK_ENABLED", False))
        else:
            precheck_enabled = str(raw_precheck_flag).strip().lower() in {
                "1",
                "true",
                "yes",
                "on",
